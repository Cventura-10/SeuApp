...
exports.createResultsAccessor = exports.Results = void 0;
 ...
 */
class Results extends internal_1.OrderedCollection {
    /** @internal */
    subscriptionName;
    /**
    ...
    constructor(realm, internal, accessor, typeHelpers) {
        if (arguments.length === 0 || !(internal instanceof internal_1.binding.Results)) {
            throw new internal_1.IllegalConstructorError("Results");
        }
     ...
     */
    description() {
        return internal_1.binding.Helpers.getResultsDescription(this.internal);
    }
    /**
    ...
    update(propertyName, value) {
        internal_1.assert.string(propertyName);
        const { classHelpers, type, results } = this;
        (0, internal_1.assert)(type === "object" && classHelpers, "Expected a result of Objects");
        ...
        for (let i = 0; i < size; i++) {
            ...
            internal_1.assert.instanceOf(obj, internal_1.binding.Obj);
            objectSet(obj, value);
        }
    }
    /**
     ...
     */
    async subscribe(options = { behavior: internal_1.WaitForSync.FirstTime }) {
        const subs = this.realm.subscriptions;
        const shouldWait = options.behavior === internal_1.WaitForSync.Always || (options.behavior === internal_1.WaitForSync.FirstTime && !subs.exists(this));
        if (shouldWait) {
            if (typeof options.timeout === "number") {
                await new internal_1.TimeoutPromise(subs.update((mutableSubs) => mutableSubs.add(this, options)), { ms: options.timeout, rejectOnTimeout: false });
            }
            else {
        ...
        get(results, index) {
            ...
            switch (value) {
                case internal_1.binding.ListSentinel: {
                    const accessor = (0, internal_1.createListAccessor)({ realm, typeHelpers, itemType: 9 /* binding.PropertyType.Mixed */ });
                    return new internal_1.List(realm, results.getList(index), accessor, typeHelpers);
                }
                case internal_1.binding.DictionarySentinel: {
                    ...
                    return new internal_1.Dictionary(realm, results.getDictionary(index), accessor, typeHelpers);
                }
                default:
...
function createResultsAccessorForKnownType({ typeHelpers, itemType, }) {
    return {
        get: (0, internal_1.createDefaultGetter)({ fromBinding: typeHelpers.fromBinding, itemType }),
    };
}
